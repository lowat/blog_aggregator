...
const createFeed_Follow = `-- name: CreateFeed_Follow :one
INSERT INTO
    feed_follows (
...
type CreateFeed_FollowParams struct {
	ID        uuid.UUID
	CreatedAt time.Time
	UpdatedAt time.Time
	UserID    uuid.UUID
...
func (q *Queries) CreateFeed_Follow(ctx context.Context, arg CreateFeed_FollowParams) (FeedFollow, error) {
	row := q.db.QueryRowContext(ctx, createFeed_Follow,
		arg.ID,
		arg.CreatedAt,
		arg.UpdatedAt,
		arg.UserID,
